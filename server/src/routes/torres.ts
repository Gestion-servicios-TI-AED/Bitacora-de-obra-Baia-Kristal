...
router.post('/', authenticateToken, requireRole('admin'), async (req: AuthRequest, res: Response) => {
    try {
        const { nombre, proyectoId, abreviatura } = req.body;
        const torre = await prisma.torre.create({ data: { nombre, abreviatura: abreviatura || null, proyectoId } });
...
router.put('/:id', authenticateToken, requireRole('admin'), async (req: AuthRequest, res: Response) => {
    try {
        const { nombre, activo, abreviatura } = req.body;
        const data: any = {};
